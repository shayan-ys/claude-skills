...
1. Read `${user_config.wikiRoot}/AGENTS.md` (or `CLAUDE.md` if the vault uses that name) — this is the authoritative schema. Every check below validates against it.
...
## Step 1 — Parallel Scanning (optional)

For a full-vault lint, if your harness supports subagents, spawn one per vault area; otherwise run serially. Each subagent runs the checks from Step 2 on its assigned folders and reports its findings back.
...
For a scoped lint (single folder), skip subagents and run checks directly.
...
3. Report findings as structured lists back to the coordinator

After all subagents report back, the coordinator (you) aggregates findings into the summary in Step 3.
...
For each note, verify against the schema in `${user_config.wikiRoot}/AGENTS.md`:
...
For notes with types that have a heading contract (article, research, project, idea), verify they have the expected H2 sections per `${user_config.wikiRoot}/AGENTS.md`'s Heading Structure Contract table.
...
- Tags outside the established namespaces in `${user_config.wikiRoot}/AGENTS.md`
...
2. If the ledger exists, flag entries stale by age; confirm with the user before bulk review.
...
### 2o. Note history and recovery

Use these recipes when a check finds damaged or unexpectedly changed content. They need the local Obsidian CLI; skip them if it is unavailable.

```bash
obsidian vault="${user_config.obsidianVaultName}" search "query here"
obsidian vault="${user_config.obsidianVaultName}" history path="folder/note.md"
obsidian vault="${user_config.obsidianVaultName}" history:read path="folder/note.md" version=2
obsidian vault="${user_config.obsidianVaultName}" diff path="folder/note.md" from=2 to=1
```

Do not keep a `log.md` change log in the vault: File Recovery and Sync history already record every version.
...
When fixing, reuse parallel subagents if there are many fixes across different areas. Each subagent handles its own folder's fixes and updates the relevant `_index.md` when done.
...
1. Close any subagents you started.
2. Give the user a brief summary: "Fixed X issues, Y items need your decision, Z items skipped."
